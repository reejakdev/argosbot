...
export function upsertEntity(
  ...
  if (existing) {
    db.prepare(
      'UPDATE entities SET last_seen = ?, properties = ?, source_ref = ? WHERE id = ?',
...
export function addRelation(
  ...
  const exists = db
    .prepare('SELECT id FROM entity_relations WHERE from_id=? AND to_id=? AND relation=?')
...
export function queryGraph(entityName: string): GraphQueryResult | null {
  ...
  const entity = db
    .prepare('SELECT id, type, name, properties FROM entities WHERE name = ? LIMIT 1')
  ...
  return {
    ...
    related: [
      ...outbound.map((r) => ({
        ...
        entity: { id: r.id, type: r.type, name: r.name },
        context: r.context,
      })),
      ...inbound.map((r) => ({
        relation: r.relation,
