...
async function runSubAgent(
  ...
  log.debug(`[${runId}] Sub-agent "${task.name}" starting — tools: ${task.tools.join(', ') || 'all'}`);
      ...
      for (const tc of step.toolCalls) {
        ...
        try {
          if (builtinNames.has(tc.name)) {
            ...
            const r = await executeSkill(tc.name, tc.input as Record<string, unknown>, config.skills);
  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error(`Sub-agent "${task.name}" timed out after ${resolvedTimeout}ms`)), resolvedTimeout),
  );
