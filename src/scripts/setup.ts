...
async function stepWebApp(total: number, config: Record<string, unknown>): Promise<void> {
  ...
  if (webapp.webauthnRpId && webapp.webauthnOrigin) {
  ...
  console.log(box([
    ...
    `  ${c.gray}WebAuthn (YubiKey) requires HTTPS on any non-localhost origin.${c.reset}`,
    '',
  ], W - 4));
  nl();
  ...
  const mode = await select('Access mode', [
    ...
    { label: 'LAN  (mkcert TLS)',  value: '2', hint: 'Phone on same WiFi — one-time cert setup' },
    { label: 'Tailscale  (HTTPS)', value: '3', hint: 'Easiest — any device, automatic HTTPS' },
  ]);
  ...
  if (mode === '1') {
    ...
    tutorial('LAN access with mkcert (one-time)', [
      '1. Install mkcert:   brew install mkcert && mkcert -install',
      `2. Generate cert:    mkcert ${lanIp ?? '192.168.x.x'}`,
      '3. Argos will use the generated .pem files',
      '4. Open phone:       https://<your-ip>:<port>  → trust the cert if prompted',
    ]);

    const ip = await ask('Your Mac LAN IP', lanIp ?? '');
    if (!ip) {
      ...
      webappCfg.tlsCert = await ask('TLS cert file (.pem)', `./${ip}.pem`);
      webappCfg.tlsKey  = await ask('TLS key file (.pem)', `./${ip}-key.pem`);
      ok(`LAN mode configured — ${webappCfg.webauthnOrigin}`);
      note(`Run:  mkcert ${ip}  then start Argos`);
    }

  } else if (mode === '3') {
    tutorial('Tailscale access (easiest — HTTPS automatic)', [
      '1. Install Tailscale on Mac + phone → tailscale.com',
      '2. Enable Tailscale Funnel (or just use your tailnet hostname)',
      '3. Your hostname:  hostname.tailXXXX.ts.net  (visible in Tailscale app)',
    ]);
    ...
    if (hostname) {
      ...
      ok(`Tailscale mode — ${webappCfg.webauthnOrigin}`);
    } else {
...
Produce ONLY a valid JSON object with this exact structure:
{
  ...
  "anonymizer": { "mode": "regex", "knownPersons": [], "knownAddresses": [], "bucketAmounts": true },
...
import type { McpCatalogEntry, McpCategory } from '../mcp/index.js';
