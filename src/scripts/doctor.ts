...
function checkSecretsStore(secrets: Record<string, string>): void {
  ...
  const label    = backend === 'keychain' ? `system keychain  ${c.gray}(macOS Keychain / GNOME Keyring)${c.reset}` : `file  ${c.gray}(~/.argos/secrets.json)${c.reset}`;
  if (count === 0) {
...
function checkConfig(secrets: Record<string, string>): void {
  ...
  if (!owner?.name) {
    ...
    warn('Config — owner.telegramUserId', 'still 0 — update with your Telegram user ID',
  ...
  if (!llm?.activeProvider) {
    ...
    if (!provCfg) {
      warn(`Config — LLM provider (${providerId})`, 'provider block not configured', 'npm run setup  → step 1');
    } else if (!noKeyNeeded.has(providerId)) {
      const apiKey = resolveValue(provCfg.apiKey, secrets);
      if (!apiKey) {
        fail(`Config — LLM API key (${providerId})`, 'apiKey not set or secret not found in store', 'npm run setup  → step 1');
      } else {
        pass('Config — LLM', `${providerId}  ${model ?? '(no model set)'}  ${c.gray}(key: ✓)${c.reset}`);
      }
    } else {
...
async function checkLlmConnectivity(): Promise<void> {
  try {
    ...
    try {
      ...
      process.stdout.write('\r\x1b[K');
      fail('LLM connectivity', `API call failed: ${(e as Error).message}`, 'Check API key and network');
    }
  } catch {
...
function checkApprovalChannel(cfg: Record<string, unknown> | null, secrets: Record<string, string>): void {
  if (!cfg) { skip('Approval channel', 'config not loaded'); return; }

  const channel = cfg.channel as string | undefined;
  const channels = (cfg.channels ?? {}) as Record<string, unknown>;

  if (channel === 'telegram-bot') {
    const botToken = resolveValue(
      (secrets['TELEGRAM_BOT_TOKEN']) ? '$TELEGRAM_BOT_TOKEN' : ((cfg.secrets as Record<string, unknown> | undefined)?.TELEGRAM_BOT_TOKEN),
      secrets,
    ) ?? secrets['TELEGRAM_BOT_TOKEN'];
    if (botToken) pass('Approval channel', `Telegram Bot  ${c.gray}(token: ✓)${c.reset}`);
    else fail('Approval channel', 'Telegram Bot configured but TELEGRAM_BOT_TOKEN not found in secrets store', 'npm run setup  → step 1');
  } else if (channel === 'slack') {
    const slackBot = (channels.slack as Record<string, unknown> | undefined)?.personal as Record<string, unknown> | undefined;
    ...
    else fail('Approval channel', 'Slack Bot configured but token not found in secrets store', 'npm run setup  → step 1');
  } else if (channel === 'discord') {
    const dcToken = secrets['DISCORD_BOT_TOKEN'];
    if (dcToken) pass('Approval channel', `Discord Bot  ${c.gray}(token: ✓)${c.reset}`);
    else fail('Approval channel', 'Discord configured but DISCORD_BOT_TOKEN not found in secrets store', 'npm run setup  → step 1');
  } else if (!channel) {
...
function checkTelegramListener(cfg: Record<string, unknown> | null, secrets: Record<string, string>): void {
  if (!cfg) { skip('Telegram listener', 'config not loaded'); return; }
  const channels = (cfg.channels ?? {}) as Record<string, unknown>;
  const tg = (channels.telegram as Record<string, unknown> | undefined)?.listener as Record<string, unknown> | undefined;
  ...
  const apiId   = secrets['TELEGRAM_API_ID']   ?? resolveValue((cfg.secrets as Record<string, unknown> | undefined)?.TELEGRAM_API_ID, secrets);
  const apiHash = secrets['TELEGRAM_API_HASH']  ?? resolveValue((cfg.secrets as Record<string, unknown> | undefined)?.TELEGRAM_API_HASH, secrets);

  if (!apiId || !apiHash) {
    fail('Telegram MTProto listener', 'TELEGRAM_API_ID / TELEGRAM_API_HASH missing from secrets store',
...
async function checkDatabase(): Promise<void> {
  ...
  try {
    ...
    const props  = db.prepare("SELECT count(*) as n FROM proposals WHERE status IN ('proposed','awaiting_approval')").get() as { n: number };
    const tasks  = db.prepare("SELECT count(*) as n FROM tasks WHERE status IN ('open','in_progress')").get() as { n: number };
    db.close();
    pass('Database', `${c.gray}${tables.n} tables  ·  ${props.n} pending proposals  ·  ${tasks.n} open tasks${c.reset}`);
  } catch (e) {
...
function checkWebApp(cfg: Record<string, unknown> | null): void {
  if (!cfg) { skip('Web app', 'config not loaded'); return; }
  ...
  if (!rpId || !origin) {
    warn('Web app', 'webapp.webauthnRpId / webauthnOrigin not set',
...
async function checkAuditChain(): Promise<void> {
  ...
  try {
    ...
    const hashed = (db.prepare("SELECT count(*) as n FROM audit_log WHERE entry_hash IS NOT NULL").get() as { n: number }).n;
    db.close();
    if (hashed === total) {
      pass('Audit chain', `${total} entries  ${c.gray}(all hashed — tamper-evident)${c.reset}`);
    } else if (hashed === 0) {
      warn('Audit chain', `${total} entries without hash  ${c.gray}(hash chain not yet active — upgrade DB)${c.reset}`);
    } else {
      pass('Audit chain', `${total} entries  ${c.gray}(${hashed} hashed, ${total - hashed} legacy)${c.reset}`);
    }
  } catch {
...
async function checkOptionalIntegrations(cfg: Record<string, unknown> | null, secrets: Record<string, string>): Promise<void> {
  // Slack listener
  const channels = (cfg?.channels ?? {}) as Record<string, unknown>;
  ...
  if (slackEnabled && slackToken) pass('Slack listener', `user token ✓  ${c.gray}(xoxp-...)${c.reset}`);
  else if (slackEnabled && !slackToken) fail('Slack listener', 'enabled but SLACK_USER_TOKEN missing', 'npm run setup  → Slack listener');
  ...
  const emailHost = (emailCfg.imap as Record<string, unknown> | undefined)?.host as string | undefined
    ?? secrets['EMAIL_IMAP_HOST'];
  const emailUser = (emailCfg.imap as Record<string, unknown> | undefined)?.user as string | undefined
  ...
  else if (emailHost || emailUser) warn('Email IMAP', 'partially configured — missing user or password', 'npm run setup  → email step');
  else skip('Email IMAP', 'not configured');

  // WhatsApp
  try {
    await import('@whiskeysockets/baileys');
    const waEnabled = (channels.whatsapp as Record<string, unknown> | undefined)?.enabled === true
  ...
  else if (gcalId) warn('Google Calendar', 'client_id set but GOOGLE_REFRESH_TOKEN missing', 'Run OAuth flow to get refresh token');
  else skip('Google Calendar', 'not configured');

  // Linear
  const linearKey = secrets['LINEAR_API_KEY'];
  const linearTeam = secrets['LINEAR_TEAM_ID'];
  if (linearKey) pass('Linear', linearTeam ? `team=${linearTeam}` : 'key ✓  (LINEAR_TEAM_ID not set)');
...
function printSummary(): void {
  ...
  if (counts.error > 0) {
    ...
    console.log(`  ${c.yellow}Argos can start but some features are unavailable.${c.reset}  Run  ${c.cyan}npm run doctor --fix${c.reset}  for actionable steps.\n`);
  } else {
    console.log(`  ${c.green}${c.bold}All systems go.${c.reset}  Run  ${c.cyan}npm run dev${c.reset}  to start Argos.\n`);
  }
}
...
async function main(): Promise<void> {
  ...
  // Initialize secrets store (sync — reads from ~/.argos/secrets.json)
