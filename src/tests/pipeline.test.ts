...
vi.mock('../ingestion/classifier.js', () => ({
  classify: vi.fn(async (_window: ContextWindow): Promise<ClassificationResult> => {
    const content = _window.messages
      .map((m) => m.content)
      .join(' ')
      .toLowerCase();
    ...
    if (content.includes('usdc') || content.includes('transfer') || content.includes('send')) {
      return {
        ...
        tags: ['usdc', 'transfer'],
        isMyTask: true,
        taskScope: 'my_task' as const,
        ownerConfidence: 0.95,
        requiresAction: true,
        assignedTeam: 'ops',
        completedTaskIds: [],
        isDuplicate: false,
        completionSignal: 'none' as const,
    ...
    if (
      ...
      return {
        ...
        tags: ['confirmation'],
        isMyTask: false,
        taskScope: 'info_only' as const,
        ownerConfidence: 0.7,
        requiresAction: false,
        assignedTeam: null,
        completedTaskIds: [],
        isDuplicate: false,
        completionSignal: 'strong' as const,
        injectionDetected: false,
      };
    }
    return {
...
beforeAll(async () => {
  ...
  setSendToApprovalChat(async (text) => {
...
describe('partner — tx request', () => {
  it('is persisted in messages table', async () => {
    ...
    const row = getDb().prepare('SELECT * FROM messages WHERE id = ?').get(msg.id) as {
      status: string;
      content_hash: string;
    } | null;
  ...
  it('generates a task after window closes', async () => {
    ...
    const task = getDb()
      .prepare(
        `SELECT * FROM tasks WHERE chat_id = 'chat-alice-001' ORDER BY detected_at DESC LIMIT 1`,
      )
      .get() as { category: string; title: string } | null;
  ...
  it('stores a memory entry', async () => {
    const mem = getDb()
      .prepare(
        `SELECT * FROM memories WHERE chat_id = 'chat-alice-001' ORDER BY created_at DESC LIMIT 1`,
      )
...
describe('partner — KYC / compliance request', () => {
  it('creates a client_request task', async () => {
    await ingest(
      'We need your KYC documents for the onboarding by end of week.',
      'chat-bob-001',
      'Bob Martin',
    );
    await flush();
    const task = getDb()
      .prepare(
        `SELECT * FROM tasks WHERE chat_id = 'chat-bob-001' ORDER BY detected_at DESC LIMIT 1`,
      )
...
describe('partner — injection attempt', () => {
  it('is blocked before entering the pipeline', async () => {
    ...
    await ingestMessage(msg, llmConfig, null, config, anonymizer, windowManager);
    const row = getDb().prepare('SELECT status FROM messages WHERE id = ?').get(msg.id) as {
      status: string;
    } | null;
  ...
  it('blocked message creates no memory or task', async () => {
    const mem = getDb()
      .prepare(`SELECT COUNT(*) as c FROM memories  WHERE chat_id = 'chat-evil-001'`)
      .get() as { c: number };
    const task = getDb()
      .prepare(`SELECT COUNT(*) as c FROM tasks     WHERE chat_id = 'chat-evil-001'`)
      .get() as { c: number };
    expect(mem.c).toBe(0);
...
describe('owner — completion signal', () => {
  it('is classified as info, no task created', async () => {
    ...
    const tasks = getDb()
      .prepare(`SELECT * FROM tasks WHERE chat_id = 'chat-alice-001' ORDER BY detected_at DESC`)
...
describe('privacy — PII anonymization', () => {
  it('email is replaced by placeholder before storage', async () => {
    ...
    const row = getDb().prepare('SELECT anon_lookup FROM messages WHERE id = ?').get(msg.id) as {
...
describe('privacy — chatId isolation', () => {
  it('two different chats do not share context', async () => {
    ...
    // Both are stored
    const aliceMsgs = getDb()
      .prepare(`SELECT COUNT(*) as c FROM messages WHERE chat_id = 'chat-isolation-alice'`)
      .get() as { c: number };
    const bobMsgs = getDb()
      .prepare(`SELECT COUNT(*) as c FROM messages WHERE chat_id = 'chat-isolation-bob'`)
      .get() as { c: number };
    ...
    const aliceMemories = getDb()
      .prepare(`SELECT * FROM memories WHERE chat_id = 'chat-isolation-alice'`)
      .all();
    const bobMemories = getDb()
      .prepare(`SELECT * FROM memories WHERE chat_id = 'chat-isolation-bob'`)
      .all();
