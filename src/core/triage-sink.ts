...
async function sinkTask(
  ...
  if (existing) {
    log.debug(`Task dedup — open task ${existing.id} already exists for ${result.partner}, skipping`);
  ...
  if (config.notion && !config.readOnly) {
    const dbId = config.triage.notionTodoDatabaseId
    ...
    if (dbId) {
      try {
        ...
        await worker.createEntry({
          ...
          priority:      result.urgency === 'high' ? 'High' : result.urgency === 'medium' ? 'Medium' : 'Low',
        });
...
async function generateDraftReply(
  ...
  log.debug(`generateDraftReply: ${memories.length} memory result(s) for partner=${result.partner}`);
...
NEVER refer to yourself in third person.${memoryContext}${knowledgeBlock}`,
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, 'proposed', ?, ?)
  `).run(pid, taskId, result.body.slice(0, 500), `Draft reply to ${result.partner}`, actions, draft, now, expiresAt);

  audit('triage_draft_created', pid, 'proposal', { partner: result.partner, chatId: result.chatId });
