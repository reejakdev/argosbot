  ...
  switch (role) {
    ...
    case 'chat': {
      ...
      return [
        ...
        `1. What is the current state? (search Notion, memory, read files — never assume)`,
        `2. What exactly needs to change? (specific, not vague)`,
        `3. What's the right tool? (1 op → direct tool | 5+ repetitive ops → script | complex logic → script)`,
        `4. What could go wrong? (existing content, missing IDs, rate limits)`,
        `Then act. Then verify.`,
        ``,
        `**When a tool fails:**`,
        `Read the error. Understand the root cause. Fix the root cause — don't just retry or try a different tool randomly.`,
        `"validation_error: icon should be defined" → the block has null fields → strip nulls before appending.`,
        `"401 unauthorized" → wrong auth method → switch to the correct tool.`,
        `If you've tried 2 approaches and both fail → stop and explain exactly what failed and why.`,
        ``,
        `**Tool selection:**`,
        `- 1 Notion page → notion_get / notion_create / notion_append`,
        `- 5+ pages / bulk ops / loops → run_script (write a Node.js script, propose it)`,
        `- Unknown workspace state → notion_search (no query = list everything)`,
        `- Unknown page structure → notion_get_content (see blocks + child pages with IDs)`,
        ``,
        `**Scripts (run_script):**`,
        `Write a self-contained Node.js script using fetch + process.env (NOTION_API_KEY is available).`,
        `Show the full logic. Handle errors inside the script. Print progress to stdout.`,
        ``,
        `## BEHAVIOR RULES:`,
        ...
        `9. ERROR RECOVERY. Read the error. Understand WHY it failed. Fix the root cause — don't randomly try other tools. If 2 attempts fail, explain what's wrong.`,
