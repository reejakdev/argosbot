...
export function store(
  ...
  `,
    ...
    entry.expiresAt,
    entry.createdAt,
    options.storeRaw ? (window.rawContent ?? null) : null,
  );
  ...
  setImmediate(() => {
    vectorizeMemoryAsync(entry).catch(() => {});
  });
...
export function search(options: SearchOptions): MemoryEntry[] {
  ...
  `,
    ...
    .all(
      ...
      now,
      !options.partnerName ? 1 : 0,
      options.partnerName ?? null,
      !options.chatId ? 1 : 0,
      options.chatId ?? null,
...
export async function vectorizeConversationAsync(
  ...
  try {
    ...
    const tags = [
      'conversation',
      result.category,
      ...(window.partnerName ? [window.partnerName] : []),
    ].filter(Boolean) as string[];

    const chunks = chunkText(text, sourceRef, window.partnerName ?? window.chatId, tags, {
